#!/usr/bin/env python3
# to run the tool sudo ./subdomain.py domain_name.com

# dns.python for dns resolution
import dns.resolver
import sys
...
def subdomain_finder():
    # this list will have all valid subdomains
    subdomains = []

    # loop through all possible subdomains in subdomains_list
    for subdomain in subdomain_list:
        try:
            # resolve ipv4 dns record
            ip_v4_record = dns.resolver.resolve(
                f'{subdomain}.{domain_name}', 'A')

            # if the ipv4 record found
            if ip_v4_record:
                full_domain = f'{subdomain}.{domain_name}'

                # add valid domains to subdomains list
        ...
        # if dns resolver cannot resolve a subdomain
        # in other words if dns query name does not exist
        except dns.resolver.NXDOMAIN:
            pass
        # if it cannot find an A record for a subdomain
